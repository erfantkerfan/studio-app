...
def start_axis(message):
    ...
    for file in [item.name for item in os.scandir(path_studio) if item.is_file()]:
        if file.endswith(('.mkv', '.MKV')):
            ...
            out_mp4 = os.path.join(path_studio, 'done', mp4)
    ...
    for file in [item.name for item in os.scandir(PATH_RABIEA) if item.is_file()]:
        if file.endswith(('.mp4', '.MP4')):
            ...
            if message['tag'] == 'rabiea':
                command = PATH_FFMPEG + ' -y -i \"' + in_mp4 + '\" -metadata title="@alaa_sanatisharif" -sws_flags lanczos -s 1280x720 -profile:v baseline -level 3.0 -vcodec libx264 -crf 19 -r 24 -preset ultrafast -pix_fmt yuv420p -tune film -acodec libfdk_aac -ab 128k -movflags +faststart \"' + out_mp4 + '\" -threads 23'
